...
void pu0to200() {
    ...
    // グラフの設定
    TGraph *g1 = new TGraph();  // ttbar (PU200)
    ...
    for (int bin_num = min_bin_num; bin_num <= max_bin_num; bin_num *= 2) {
        ...
        double efficiency = static_cast<double>(true_count) / (true_count + false_count);
        g3->SetPoint(g3->GetN(), bin_width, efficiency);
    ...
    for (int bin_num = min_bin_num; bin_num <= max_bin_num; bin_num *= 2) {
        ...
        double efficiency = static_cast<double>(true_count) / (true_count + false_count);
        g4->SetPoint(g4->GetN(), bin_width, efficiency);
